...
export function createFirstChapterBrandSystem({ stage, scene, canvas, lightingRig }) {
  ...
  const baseFog = scene.fog
    ...
    : null;
  const refinedFog = new THREE.Color(0x60404a);

  const lights = lightingRig ? {
    ambient: {
      light: lightingRig.ambient,
      color: lightingRig.ambient.color.clone(),
      intensity: lightingRig.ambient.intensity,
      target: new THREE.Color(0x342035),
      targetIntensity: 0.07
    },
    sky: {
      light: lightingRig.sky,
      color: lightingRig.sky.color.clone(),
      groundColor: lightingRig.sky.groundColor.clone(),
      intensity: lightingRig.sky.intensity,
      target: new THREE.Color(0xff896e),
      targetGround: new THREE.Color(0x0d0710),
      targetIntensity: 0.2
    },
    moon: {
      light: lightingRig.moon,
      color: lightingRig.moon.color.clone(),
      intensity: lightingRig.moon.intensity,
      target: new THREE.Color(0x74405f),
      targetIntensity: 0.07
    },
    rim: {
      light: lightingRig.rim,
      color: lightingRig.rim.color.clone(),
      intensity: lightingRig.rim.intensity,
      target: new THREE.Color(0xff3d8c),
      targetIntensity: 1.28
    },
    warmBounce: {
      light: lightingRig.warmBounce,
      color: lightingRig.warmBounce.color.clone(),
      intensity: lightingRig.warmBounce.intensity,
      target: new THREE.Color(0xff9a55),
      targetIntensity: 0.58
    },
    district: lightingRig.districtLights.map(item => ({
      light: item,
  ...
  function applyLights(p) {
    ...
    lights.district.forEach((entry, index) => {
      entry.light.color.copy(entry.color).lerp(index % 2 ? new THREE.Color(0xff5b78) : new THREE.Color(0xff9a58), p * 0.28);
  ...
  function update(delta, controller) {
    ...
    progress = THREE.MathUtils.lerp(
      progress,
      target,
      1 - Math.exp(-response * Math.max(0.001, delta))
    );
    if (Math.abs(progress - target) < 0.001) progress = target;

    const p = clamp01(progress);
    field.style.opacity = String(p * 0.06);
    haze.style.opacity = String(p * 0.2);
    light.style.opacity = String(p * 0.6);
    stage.style.setProperty("--brand-progress", p.toFixed(4));
    stage.classList.toggle("is-brand-freeze", p > 0.015);

    const saturation = THREE.MathUtils.lerp(0.78, 0.66, p);
    const contrast = THREE.MathUtils.lerp(1.14, 1.24, p);
    const brightness = THREE.MathUtils.lerp(0.84, 0.77, p);
    ...
    if (baseFog && scene.fog) {
      scene.fog.color.copy(baseFog.color).lerp(refinedFog, p * 0.38);
      scene.fog.density = THREE.MathUtils.lerp(baseFog.density, baseFog.density * 0.8, p);
    }
    ...
    window.__h1V3BrandTransitionAudit = {
      ...
      environmentShift: "backlit-sunset-city-to-clean-brand-freeze",
      citySaturation: Number(saturation.toFixed(3)),
      cityContrast: Number(contrast.toFixed(3)),
      overlayColorWash: false,
      hiddenCut: false,
      materialMode: "deep-plum-shared-geometry-rim"
    };
  }
